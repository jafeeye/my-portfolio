...
1. 先建立一個LXC的非特權容器開啟Nesting，安裝powerppc模擬器
```
apt update
apt install qemu-system-ppc qemu-utils -y
```
2. 掛載ISO檔案，nano /etc/pve/lxc/容器ID.conf
```
...
5. 要放到腳本上執行不然會斷線，建立nano start_aix.sh 存檔後給權限 `chmod +x start_aix.sh`
```bash
#!/bin/bash
...
# 把這個網橋介面「升起來」（啟動） 
ip link set br0 up
#建立你在 QEMU 寫的那個 tap0 網卡 
ip tuntap add dev tap0 mode tap 
ip link set tap0 up
#把 tap0 和容器原本的網卡 eth0 全部扔進網橋裡 
brctl addif br0 tap0 
brctl addif br0 eth0
